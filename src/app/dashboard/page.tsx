      ...
      try {
        ...
        setStats([
          ...
          },
          {
            ...
            value: condominios.filter((c: any) => c.ativo !== false).length,
            icon: <TrendingUp className="w-6 h-6" />,
            color: "bg-green-50 text-green-600 border-green-200",
            description: "Condomínios em operação",
          },
          {
        ...
        {stats.map((stat, index) => (
          <Card key={index}>
            <CardBody className="flex flex-col">
              <div className={`inline-flex w-12 h-12 items-center justify-center rounded-lg mb-4 ${stat.color}`}>
                {stat.icon}
              </div>
